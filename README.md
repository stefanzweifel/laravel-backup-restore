...
The test suite runs on Linux, macOS and Windows. The database importers still shell out to `gunzip` and `bunzip2`, so the tests that import a compressed dump do not pass on Windows yet.
